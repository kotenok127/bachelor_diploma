...
newDir = os.path.join('.', 'large_nf_tsne_pca_thesis')
nset = 'large_set1'
...
def tsne_gen(XX, pref, lr, perp):
    ...
    print(pref, lr, perp, quality(X_reduced, label))
                    # if (dist(label[j], ))

    plt.savefig(os.path.join(newDir, ('pref_lr=' +str(lr) + '_perp=' + str(perp)+  '.png')))
    # plt.show()
    plt.close()


def gen_tse_with_pca(XX, pref):
    for lr in [1000, 500, 200, 50, 10]:
        for perp in [100, 30, 5]:
            tsne_gen(XX, pref + "_all_", lr, perp)
            for n_pca in [5, 10, 25]:
                pca = PCA(n_components=n_pca)
                X_pca = pca.fit_transform(XX)
                tsne_gen(XX, pref + "_pca" + str(n_pca) +"_", lr, perp)
